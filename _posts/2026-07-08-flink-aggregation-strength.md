...
그 작업의 금액 보정 로직은 이 장점을 잘 보여준다. `(partner_idx, order_id, client_type)` 단위로 주문 아이템들을 MapState에 보관하고, 결제 금액이 비어 있거나 취소/삭제 이벤트가 들어오면 활성 아이템들의 `paymentPrice`를 다시 합산한다. 그리고 그 합계를 같은 주문의 모든 아이템에 다시 emit한다. 외부 DB를 매번 조회해서 맞추는 방식이 아니라, Flink operator 안의 keyed state가 현재 주문의 부분 집계를 들고 있는 구조다.
